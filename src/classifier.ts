...
export function detectComplexity(prompt: string): Complexity {
  ...
  const hasMultiGoal =
    (prompt.match(/\b(also|additionally|furthermore|and then|as well as)\b/gi) || []).length > 1
...
const AMBIGUITY_INDICATORS: Array<{ pattern: RegExp; dimension: string }> = [
  {
    pattern: /\b(something|somehow|kind of|sort of|maybe|perhaps|probably)\b/i,
    dimension: 'vague intent',
  },
...
export function extractSlots(prompt: string): ArgumentSlots {
  ...
  const refMatches =
    prompt.match(/(?:https?:\/\/\S+|\/[\w/.-]+\.\w+|`[^`]+`|"[^"]+"|\bv\d+\.\d+[.\d]*\b)/g) || []
...
// ---------------------------------------------------------------------------
// Short-prompt intent detection + expansion
// ---------------------------------------------------------------------------

/**
 * Verb-first intent patterns for short prompts.
 * If the prompt matches, it is expanded into a structured template that gives
 * the main model enough context to produce a useful answer without requiring
 * the user to elaborate first.
 */
interface IntentPattern {
  pattern: RegExp
  intent: string
  template: (match: RegExpMatchArray, ctx: string) => string
}
...
const INTENT_PATTERNS: IntentPattern[] = [
  {
    ...
    intent: 'fix',
    template: (m, ctx) =>
      `Fix the issue with ${m[1].trim()}. Identify the root cause, explain what is wrong, and provide the corrected code.${ctx}`,
  },
  {
    pattern: /^explain\s+(.+)/i,
    intent: 'explain',
    template: (m, ctx) =>
      `Explain how ${m[1].trim()} works. Cover the key logic, data flow, and any non-obvious behaviour.${ctx}`,
  },
  {
    pattern: /^(?:refactor|clean up|cleanup)\s+(.+)/i,
    intent: 'refactor',
    template: (m, ctx) =>
      `Refactor ${m[1].trim()}. Improve readability and reduce complexity while maintaining the same behaviour. Show before and after.${ctx}`,
  },
  {
    pattern: /^add\s+(.+)/i,
    intent: 'add',
    template: (m, ctx) =>
      `Add ${m[1].trim()}. Follow the existing code conventions and patterns in this project.${ctx}`,
  },
  {
    pattern: /^(?:remove|delete)\s+(.+)/i,
    intent: 'remove',
    template: (m, ctx) =>
      `Remove ${m[1].trim()}. Check that nothing else depends on it before deletion and clean up any related references.${ctx}`,
  },
  {
    pattern: /^(?:test|write tests? for|add tests? for)\s+(.+)/i,
    intent: 'test',
    template: (m, ctx) =>
      `Write tests for ${m[1].trim()}. Cover the happy path, edge cases, and error conditions.${ctx}`,
  },
  {
    pattern: /^debug\s+(.+)/i,
    intent: 'debug',
    template: (m, ctx) =>
      `Debug ${m[1].trim()}. Trace the execution path, identify where it fails, and explain why.${ctx}`,
  },
  {
    pattern: /^(?:update|change|modify)\s+(.+)/i,
    intent: 'update',
    template: (m, ctx) =>
      `Update ${m[1].trim()}. Describe the change needed, show what to modify, and ensure existing behaviour is preserved unless explicitly changed.${ctx}`,
  },
  {
    pattern: /^(?:implement|create|build|write)\s+(.+)/i,
    intent: 'implement',
    template: (m, ctx) =>
      `Implement ${m[1].trim()}. Follow the existing code conventions, include type annotations, and add brief comments for non-obvious logic.${ctx}`,
  },
  {
    pattern: /^(?:review|check|audit)\s+(.+)/i,
    intent: 'review',
    template: (m, ctx) =>
      `Review ${m[1].trim()}. Identify issues, suggest improvements, and flag anything that looks incorrect or fragile.${ctx}`,
  },
]

/**
 * Attempt to expand a short prompt using verb-first intent templates.
 *
 * Returns `{ expanded, matched: true }` when a pattern fires.
 * Returns `{ expanded: prompt, matched: false }` when no pattern matches —
 * the caller should decide whether to skip enhancement entirely.
 *
 * @param prompt      The raw user prompt (should be ≤ ~6 words)
 * @param contextHint Optional one-liner appended after the expanded template
 *                    (e.g. "Current branch: feat/login")
 */
export function expandShortPrompt(
  prompt: string,
  contextHint = ''
): { expanded: string; matched: boolean; intent: string | null } {
  const trimmed = prompt.trim()
  const words = trimmed.split(/\s+/)

  // Single-word prompts with no recognised verb are non-actionable (e.g. "hello", "ok")
  if (words.length === 1) {
    const singleVerbMatch = INTENT_PATTERNS.find(p => p.pattern.test(trimmed))
    if (!singleVerbMatch) return { expanded: trimmed, matched: false, intent: null }
  }

  const ctxSuffix = contextHint ? `\n\nContext: ${contextHint}` : ''

  for (const { pattern, intent, template } of INTENT_PATTERNS) {
    const match = trimmed.match(pattern)
    if (match) {
      return { expanded: template(match, ctxSuffix), matched: true, intent }
    }
  }

  return { expanded: trimmed, matched: false, intent: null }
}

// ---------------------------------------------------------------------------
// Generate clarifying questions for ambiguous prompts
// ---------------------------------------------------------------------------
export function generateClarifyingQuestions(classification: ClassificationResult): string[] {
  ...
  if (ambiguousDimensions.includes('very short / underspecified')) {
    questions.push(
      'Could you expand on your request? The more context you provide, the better the result.'
    )
  }
