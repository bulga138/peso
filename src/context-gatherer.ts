...
export function gatherContext(cwd?: string, mcpTools?: string[], toolPriorities?: ToolPriorities, contextConfig?: ContextConfig): WorkspaceContext {
  ...
  return {
    ...
    gitChangedFiles: cfg.injectGitChangedFiles ? cache.getChangedFiles(effectiveCwd).slice(0, cfg.maxChangedFiles ?? 10) : [],
...
export function formatContextBlock(ctx: WorkspaceContext, prompt: string): string {
  ...
  if (toolPrioritiesCache) {
    let preferred = [...toolPrioritiesCache.prefer];
