...
import { checkAllRules } from "./rules/research-backed.js";
import { autoSelectTechniques, applyTechniques, TECHNIQUES, resolveModelTier } from "./techniques.js";
import { gatherContext, formatContextBlock, detectStaleInfo, type ContextConfig as GathererContextConfig } from "./context-gatherer.js";
...
function stageInjectContext(
  ...
  const ctx = gatherContext(options.cwd, options.mcpTools, options.toolPriorities, options.contextConfig);
...
function stageValidate(original: string, enhanced: string): { result: string; note: string } {
  ...
  const enhWords = new Set(
    enhanced.toLowerCase().split(/\W+/).filter((w) => w.length > 3)
  );
  ...
  if (ratio < 0.5) {
    return { result: enhanced, note: `WARNING: low intent preservation (${Math.round(ratio * 100)}%)` };
  }
  ...
  const bash: "ask" | "allow" | "deny" =
    typeof bashRaw === "string"
      ...
      : (bashRaw != null && typeof bashRaw === "object"
          ? (Object.values(bashRaw as Record<string, string>)[0] as "ask" | "allow" | "deny" | undefined) ?? "ask"
          : "ask");
...
export function runPipeline(
  ...
  const intensity = resolveIntensity(classification.complexity, options.agent, options.agentPermissions);
  ...
  if (intensity === "none") {
    ...
    return {
      ...
      stages: [{ stage: "analyze", applied: false, change: `skipped: agent "${options.agent}" classified as explore (read-only+broad)` }],
      techniquesApplied: [],
  ...
  // --- Stage 1: Analyze
  stages.push({ stage: "analyze", applied: true, change: `baseline scored, intensity: ${intensity}${options.agent ? ` (agent: ${options.agent})` : ""}` });
  ...
  if (!skip.has("apply-techniques")) {
    ...
    const userTechIds = (options.selectedTechniqueIds || 
      autoSelectTechniques(prompt, domain, options.modelId).map((t) => t.id)
    ).filter((id) => !SYSTEM_TECHNIQUES.has(id));

    if (userTechIds.length > 0) {
      const enhanced = applyTechniques(current, userTechIds, domain, options.modelId);
      ...
      stages.push({ stage: "apply-techniques", applied: false, change: "no user-level techniques applicable" });
    }
  }
