...
import { classify, generateClarifyingQuestions, expandShortPrompt } from './classifier.js'
import { runPipeline, initTechniquePacks } from './enhancer.js'
...
import { loadPesoConfig, discoverGlobalPacks, type PesoConfig } from './config.js'
...
async function resolveToolListModel(
  ...
  try {
    ...
    if (smallModelStr && smallModelStr.includes('/')) {
      ...
      return {
        providerID: smallModelStr.substring(0, slash),
        modelID: smallModelStr.substring(slash + 1),
      }
    }

    // Priority 2: top-level model
    const modelStr: string | undefined =
      typeof cfgData?.model === 'string' ? cfgData.model : undefined
...
// ---------------------------------------------------------------------------
// Enhancement system prompts — generic fallback + per-domain variants
// ---------------------------------------------------------------------------
...
/**
 * Domain-specific rewrite system prompts.
 * Used by enhanceWithSdk() when the pipeline did not produce a system prompt.
 * Each variant focuses on what matters most for that domain.
 */
const DOMAIN_REWRITE_PROMPTS: Record<Domain, string> = {
  code: `You are PESO, a prompt optimizer for AI coding assistants. Rewrite the user's prompt to be:
- Specific about the target file, function, or module
- Clear about the expected output format (code, explanation, diff)
- Explicit about constraints (language, framework, style conventions)
- Front-loaded with the most critical instruction
Rules:
- Preserve original intent exactly — do NOT invent requirements
- Use short, direct words
- Output ONLY the enhanced prompt, no meta-commentary`,

  research: `You are PESO, a prompt optimizer for AI research assistants. Rewrite the user's prompt to be:
- Specific about the depth and scope of research needed
- Clear about what format the answer should take (summary, comparison, pros/cons list)
- Explicit about any sources, time ranges, or domains to focus on
Rules:
- Preserve original intent exactly — do NOT invent requirements
- Output ONLY the enhanced prompt, no meta-commentary`,

  creative: `You are PESO, a prompt optimizer for AI creative assistants. Rewrite the user's prompt to be:
- Clear about tone, style, voice, and intended audience
- Specific about length, format, and structural constraints
- Explicit about what makes a good vs poor output for this task
Rules:
- Preserve original intent exactly — do NOT invent requirements
- Output ONLY the enhanced prompt, no meta-commentary`,

  general: ENHANCE_SYSTEM_PROMPT,
}
...
async function enhanceWithSdk(
  ...
  // Use domain-specific system prompt when no pipeline system prompt was provided
  const effectiveSystem = systemPrompt || DOMAIN_REWRITE_PROMPTS[domain] || ENHANCE_SYSTEM_PROMPT
  const userMessage = `<context>\n${contextBlock}\n</context>\n\n<domain>${domain}</domain>\n\n<original-prompt>\n${prompt}\n</original-prompt>\n\nRewrite this prompt for an AI ${domain === 'code' ? 'coding' : domain} assistant.`
  try {
    const result = await callSmallModelViaSdk(client, effectiveSystem, userMessage)
...
async function enhanceWithHttp(
  ...
  const userMessage = `<context>\n${contextBlock}\n</context>\n\n<domain>${domain}</domain>\n\n<original-prompt>\n${prompt}\n</original-prompt>\n\nRewrite this prompt for an AI ${domain === 'code' ? 'coding' : domain} assistant.`
  try {
    const result = await callSmallModel(effectiveSystem, userMessage)
    return result.text.trim() || prompt
  } catch (err) {
...
/**
 * Quality gate: compare pipeline output vs LLM rewrite and return the winner.
 *
 * Rules:
 *  - LLM rewrite wins if its score >= pipeline score − 0.5 (small tolerance for
 *    cases where the scorer doesn't capture structural improvements)
 *  - If both score lower than the original, keep the original
 *
 * Returns the winning text and a source label for debug output.
 */
function applyQualityGate(
  ...
  text: string
  source: 'llm' | 'pipeline' | 'original'
  scores: { original: number; pipeline: number; llm: number }
} {
  const originalScore = scorePrompt(original).total
  const pipelineScore = scorePrompt(pipeline).total
  const llmScore = scorePrompt(llmRewrite).total

  // LLM wins if it's not meaningfully worse than pipeline
  const winner = llmScore >= pipelineScore - 0.5 ? llmRewrite : pipeline
  const winnerScore = llmScore >= pipelineScore - 0.5 ? llmScore : pipelineScore
  const source = winner === llmRewrite ? 'llm' : 'pipeline'

  // Quality floor: if winner is worse than original, revert
  if (winnerScore < originalScore) {
    return {
      text: original,
      source: 'original',
      scores: { original: originalScore, pipeline: pipelineScore, llm: llmScore },
    }
  }
...
export const PesoPlugin: Plugin = async ({ directory, worktree, client }) => {
  ...
  // Discover and load global technique packs from ~/.config/peso/packs/
  // The project peso.json controls which packs are enabled/disabled.
  const globalPacks = discoverGlobalPacks()
  if (globalPacks.length > 0) {
    initTechniquePacks(globalPacks, pesoConfig.techniquePacks).catch(() => {})
  }
  ...
  return {
    ...
    'chat.message': async (input, output) => {
      ...
      let originalText = textParts.map(p => p.text).join('\n')
      const trimmedLen = originalText.trim().length

      // Absolute minimum: single characters, "ok", "hi", etc.
      if (trimmedLen < 5) return

      // Short-prompt handling: attempt intent-based expansion before classifying
      const wordCount = originalText.trim().split(/\s+/).length
      if (wordCount <= 6) {
        if (trimmedLen < pesoConfig.shortPromptThreshold) {
          // Below old threshold — try expansion, bail if no intent match
          const { expanded, matched } = expandShortPrompt(originalText)
          if (!matched) return
          originalText = expanded
        } else {
          // Above old threshold but still short — try expansion opportunistically
          const { expanded, matched } = expandShortPrompt(originalText)
          if (matched) originalText = expanded
        }
      }
      ...
      if (firstTextPart) {
        // Push enhanced prompt (sent to LLM)
        output.parts.push({
          ...firstTextPart,
          ...
          metadata: {
            ...
            scoreBefore: pipelineResult.scoreBefore.total,
            scoreAfter: pipelineResult.scoreAfter.total,
            techniquesApplied: pipelineResult.techniquesApplied,
          },
        } as any)

        // Push UI-only feedback line (visible to user, hidden from LLM)
        const scoreDelta = pipelineResult.scoreAfter.total - pipelineResult.scoreBefore.total
        const sign = scoreDelta >= 0 ? '+' : ''
        const techCount = pipelineResult.techniquesApplied.length
        const feedbackText =
          `✦ peso: ${pipelineResult.scoreBefore.total.toFixed(1)}→${pipelineResult.scoreAfter.total.toFixed(1)} (${sign}${scoreDelta.toFixed(1)})` +
          ` | ${techCount} technique${techCount !== 1 ? 's' : ''}` +
          ` | ${classification.domain}/${classification.complexity}`
        output.parts.push({
          ...firstTextPart,
          id: `${firstTextPart.id}-peso-feedback`,
          text: feedbackText,
          synthetic: true,
          ignored: true, // visible in UI, NOT sent to the model
        } as any)
      }
    ...
    tool: {
      peso: tool({
        ...
        args: {
          ...
          mode: tool.schema.optional(
            tool.schema.enum(['auto', 'code', 'general', 'creative', 'research'])
          ),
          use_llm: tool.schema.optional(
            tool.schema.boolean('Whether to call the small model (default: true)')
          ),
        },
        async execute(args) {
          ...
          let llmSource: 'llm' | 'pipeline' | 'original' | 'skipped' = 'skipped'
          let gateScores: { original: number; pipeline: number; llm: number } | undefined

          if (useLlm) {
            const sys =
              pipelineResult.systemPrompt ||
              DOMAIN_REWRITE_PROMPTS[classification.domain] ||
              ENHANCE_SYSTEM_PROMPT
            let llmRewrite = await enhanceWithSdk(
              client,
              prompt,
            ...
            if (llmRewrite === prompt) {
              // SDK failed — try HTTP fallback
              llmRewrite = await enhanceWithHttp(prompt, classification.domain, contextBlock, sys)
            }
            if (llmRewrite !== prompt) {
              const gate = applyQualityGate(prompt, pipelineResult.enhanced, llmRewrite)
              enhanced = gate.text
              llmSource = gate.source
              gateScores = gate.scores
            } else {
              // Both paths failed — stay with pipeline
              llmSource = 'pipeline'
            }
          }
          ...
          const lines = [
            ...
            ...(gateScores
              ? [
                  `**Quality gate:** pipeline=${gateScores.pipeline.toFixed(1)} llm=${gateScores.llm.toFixed(1)} → kept \`${llmSource}\``,
                ]
              : []),
            ``,
            `### Score`,
          ...
          if (pipelineResult.needsFreshInfo)
            lines.push(
              ``,
              `### Freshness Warning`,
              `Prompt references potentially outdated info — use search tools.`
            )
      ...
      'peso-score': tool({
        ...
        async execute(args) {
          ...
          if (score.rules.violations.length > 0) {
            ...
            for (const v of score.rules.violations)
              lines.push(`- [${v.severity}] ${v.rule}: ${v.suggestion}`)
          }
          return lines.join('\n')
        },
      }),
        ...
        async execute(args) {
          ...
          for (const a of agents) {
            ...
            const bash = !perm?.bash
              ? 'n/a'
              : typeof perm.bash === 'string'
                ? perm.bash
                : JSON.stringify(perm.bash)
          ...
          for (const s of pipelineResult.stages)
            lines.push(`- ${s.applied ? '✓' : '–'} **${s.stage}**: ${s.change}`)
          ...
          if (!useLlm) {
            ...
            const sys =
              ...
              ENHANCE_SYSTEM_PROMPT
            const cfg = resolveConfig()
            lines.push(`- System prompt: \`${classification.domain}\` variant`)
            ...
            if (llmRewrite !== prompt) {
              lines.push(
                `- Time: ${Date.now() - t2}ms | Length: ${llmRewrite.length}ch | ✓ Success`
              )
            } else {
              lines.push(`- SDK failed, trying HTTP fallback`)
              const httpResult = await enhanceWithHttp(
                prompt,
                classification.domain,
                contextBlock,
                sys
            ...
            if (llmRewrite !== prompt) {
              ...
              lines.push(`- Original:  ${gate.scores.original.toFixed(1)}/10`)
              lines.push(`- Pipeline:  ${gate.scores.pipeline.toFixed(1)}/10`)
              lines.push(`- LLM:       ${gate.scores.llm.toFixed(1)}/10`)
              lines.push(`- **Winner: \`${gate.source}\`**`)
            }
          }
      ...
      'peso-config': tool({
        description:
          'Show PESO configuration: model, API key status, loaded agents, compass classification.',
        args: {},
        async execute() {
          const cfg = resolveConfig()
          ...
          try {
            ...
            if (resolvedModel && resolvedModel.includes('/')) {
              ...
              try {
                const listResult = await client.tool.list({
                  query: { provider: p, model: m, directory: cwd },
                })
