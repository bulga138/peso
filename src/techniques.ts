 ...
 *
 * Extensibility: external technique packs can be loaded via loadTechniquePacks().
 * A pack is a .js file exporting a default TechniquePack object.
 */
...
export function resolveModelTier(modelId?: string): ModelTier {
  ...
  const smallPatterns = ['haiku', 'gpt-4o-mini', 'gemini-flash', 'phi-', '/phi', 'llama', 'mistral-7b', 'mistral-8x7b'];
...
export const TECHNIQUES: Technique[] = [
  ...
  // 6. Example inclusion
  {
    ...
    applies: p => !hasPattern(p, /\bexample\b/i, /\bfor instance\b/i, /\be\.g\b/i) && p.split(' ').length > 30,
  ...
  // Excluded for frontier models: they reason well without this nudge.
  {
    ...
    applies: p => !hasPattern(p, /step.?by.?step/i, /think through/i, /break.?down/i) && p.split(' ').length > 60,
  ...
  // Excluded for frontier models: flattery/motivation framing adds noise.
  {
    ...
    excludeTiers: ['frontier', 'standard'],
    applies: p => !hasPattern(p, /important to my/i, /matters a lot/i, /high stakes/i),
  ...
  // 9. Constraint specification → system prompt (via SYSTEM_TECHNIQUES)
  {
    ...
    applies: p => !hasPattern(p, /\bmax\b/i, /\blimit\b/i, /\bno more than\b/i, /\bwithin\b/i, /\bconcise\b/i),
  ...
  // 12. Negative example (what NOT to do)
  {
    ...
    domains: ['code', 'general'],
    applies: p =>
      hasPattern(p, /\bexample\b/i) && !hasPattern(p, /\bbad example\b/i, /\bnot like\b/i, /\bavoid this\b/i),
  ...
  // 14. Structured output request
  {
    ...
    inject: p =>
      append(p, 'Output must be valid and parseable. Do not include explanatory text inside the structured block.'),
  },
  ...
  // 15. Scope limitation
  {
    ...
    domains: ['all'],
    applies: p =>
      !hasPattern(p, /\bonly\b/i, /\bjust\b/i, /\bfocus on\b/i, /\bscoped to\b/i) && p.split(' ').length > 40,
    inject: p =>
      append(p, 'Focus only on what is explicitly asked. Do not add unsolicited suggestions or scope expansions.'),
  },
  ...
  // 16. Uncertainty acknowledgement
  {
    ...
    applies: p => hasPattern(p, /\bfact\b/i, /\baccurate\b/i, /\blatest\b/i, /\bcurrent\b/i, /\brecent\b/i),
  ...
  // 18. Tool/resource reference
  {
    ...
    inject: p =>
      append(
        p,
        'Use available tools (search, file read, code execution) to fulfil this task rather than relying solely on training knowledge.',
      ),
  },
  ...
  // 20. Brevity instruction
  {
    ...
    applies: p =>
      !hasPattern(p, /\bbrief\b/i, /\bconcise\b/i, /\bshort\b/i, /\bsummary\b/i) && p.split(' ').length < 40,
  ...
  // 23. Code quality standards
  {
    ...
    inject: p =>
      append(
        p,
        'Write clean, type-safe code with meaningful variable names. Include brief comments for non-obvious logic.',
      ),
  },
  ...
  // 24. Iteration permission
  {
    ...
    domains: ['all'],
    applies: p => !hasPattern(p, /\bask\b/i, /\bclarif\b/i, /\bquestion\b/i) && p.split(' ').length < 25,
  ...
  // 25. Output length guidance
  {
    ...
    domains: ['general', 'creative'],
    applies: p => !hasPattern(p, /\bword\b/i, /\bparagraph\b/i, /\bsentence\b/i, /\blines?\b/i),
...
export function applyTechniques(
  ...
  const allTechniques = [...TECHNIQUES, ...extraTechniques];
  let result = prompt;
  for (const id of techniqueIds) {
    ...
    const domainMatch = technique.domains.includes('all') || technique.domains.includes(domain);
...
export function autoSelectTechniques(
  ...
  const tier = resolveModelTier(modelId);
  const all = [...TECHNIQUES, ...extraTechniques];
  return all.filter(t => {
    // Skip technique if current model tier is excluded
...
// ---------------------------------------------------------------------------
// Technique Pack — external extensibility
// ---------------------------------------------------------------------------

/**
 * A technique pack is a .js (or .ts under Bun) module that exports a default
 * TechniquePack object. PESO loads these at startup from paths listed in
 * peso.json under the `techniquePacks` key.
 *
 * Example pack file (security-pack.js):
 * ```js
 * export default {
 *   name: "security-pack",
 *   version: "1.0.0",
 ...
 *       domains: ["code"],
 *       applies: (p) => /auth|login|password|token|secret/i.test(p),
 ...
 * ```
 *
 * WARNING: Pack files are executed as normal JavaScript modules. Only load
 * packs from sources you trust — treat them the same as any other dependency.
 */
...
// Session-scoped cache so packs are only imported once
let _packCache: Technique[] | null = null;

/**
 * Dynamically import technique packs listed in config.techniquePacks.
 * Returns the merged list of built-in + pack techniques.
 * Safe to call multiple times — results are cached for the session.
 */
/**
 * Load technique packs from absolute paths.
 *
 * @param packPaths   Absolute paths to .js pack files (from discoverGlobalPacks())
 * @param packConfig  Which packs are enabled/disabled (from peso.json)
 */
export async function loadTechniquePacks(
  packPaths: string[],
  ...
  const disabled = new Set(packConfig?.disabled ?? []);
  const enabledList = packConfig?.enabled ?? 'all';

  for (const absPath of packPaths) {
    try {
      // Dynamic import — no eval, executes as a real module
      const mod = await import(absPath);
      const pack: TechniquePack = mod.default ?? mod;

      if (!pack?.techniques || !Array.isArray(pack.techniques)) {
        console.warn(`[PESO] Technique pack at ${absPath} has no techniques array — skipping`);
        continue;
      }

      const packName = pack.name ?? absPath;

      // Check enabled/disabled filters against pack name
      if (disabled.has(packName)) {
        console.error(`[PESO] Pack "${packName}" is disabled — skipping`);
        continue;
      }
      if (enabledList !== 'all' && !enabledList.includes(packName)) {
        continue; // not in the allow-list, silently skip
      }
      ...
      for (const tech of pack.techniques) {
        if (
          ...
          !Array.isArray(tech.domains) ||
          typeof tech.inject !== 'function'
        ) {
          console.warn(`[PESO] Skipping invalid technique in pack "${packName}": missing required fields`);
          continue;
        }
        // Ensure applies() defaults to always-true if omitted
        if (typeof tech.applies !== 'function') tech.applies = () => true;

        // Also check per-technique disabled list (pack-name/technique-id format)
        if (disabled.has(`${packName}/${tech.id}`)) continue;

        extras.push(tech);
      }

      console.error(
        `[PESO] Loaded technique pack "${packName}" v${pack.version} (${pack.techniques.length} techniques)`,
      );
    } catch (err) {
      console.warn(`[PESO] Failed to load technique pack at ${absPath}: ${err}`);
    }
  }
...
/** Reset pack cache — useful during testing or hot-reload scenarios */
